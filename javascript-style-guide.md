...
##### [建议] 使用`2`个或者`4` 个空格做为一个缩进层级，建议使用 `2` 个空格。
...
在严格模式或考虑 IE 下使用 `delete` 时，不能被删除的属性会抛出异常，因此在不确定属性是否可以删除的情况下，建议添加 `try-catch` 块。
